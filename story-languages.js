export const storyLanguages=[
 ...
 ,{code:'ja',short:'JA',label:'Japanese',native:'日本語',html:'ja'}
];
...
export const defaultStoryLangs={study:'ru',translation:'en',ui:'ru',pedagogy:'ru'};

export function cleanStoryLang(code,fallback='ru'){return codeSet.has(code)?code:fallback;}
...
export function normalizeStoryLangs(input=defaultStoryLangs){
 const study=cleanStoryLang(input?.study??input?.source,defaultStoryLangs.study);
 const translation=compatibleTarget(study,cleanStoryLang(input?.translation??input?.target,defaultStoryLangs.translation));
 const ui=cleanStoryLang(input?.ui,defaultStoryLangs.ui);
 const pedagogy=cleanStoryLang(input?.pedagogy,defaultStoryLangs.pedagogy);
...
export function storySupports(story,lang){return story?.i18n?.[lang]===true&&validateStoryLocale(story,lang).length===0;}
...
const explanationCopy={
 ...
 ja:{before:'なぜこの表現が合うのか：',meaning:'意味：',after:'この台詞は状況に合っています。ほかの選択肢では意味が変わります。'}
};
