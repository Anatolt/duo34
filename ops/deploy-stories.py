...
ARCHIVE=PRIVATE+'/archive/three-stories-20260910'
def encode(data):return base64.b64encode(data).decode()
def old(name):return subprocess.check_output(['git','show','203b7ec:'+name],cwd=ROOT)
...
newclips={c['src']:c for key,c in manifest['clips'].items() if key.split(':')[0] in ['hot-mic','dad-dating','return-hoodie']}
files=[]
for src,clip in newclips.items():files.append({'path':PUBLIC+'/'+src,'old':None,'data':encode((ROOT/src).read_bytes()),'sha256':clip['sha256']})
files.append({'path':PUBLIC+'/stories-new.js','old':None,'data':encode((ROOT/'dist/stories-new.js').read_bytes())})
files.append({'path':PRIVATE+'/catalog.json','old':encode(old('server/catalog.json')),'data':encode((ROOT/'server/catalog.json').read_bytes())})
# Publish dependencies before stories.js exposes the three new stories.
for name in ['story-ui.js','audio-manifest.js','app.js','stories.js']:
...
try:
 for f in files:
  p=pathlib.Path(f['path']);temp=p.with_name(p.name+'.three-stories-20260910')
