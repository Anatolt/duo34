...
export const spokenStoryLanguages = ['en', 'ru', 'es', 'zh', 'uk'];
export function supportsStoryAudio(story, lang) {
  return lang === 'en' || lang === 'ru' || (spokenStoryLanguages.includes(lang) && story?.i18n?.[lang] === true);
}
export function supportsNodeAudio(story,node,lang){
  ...
  return !!(lang==='en'?node.en:lang==='ru'?node.ru:node.i18n?.line?.[lang]);
}
...
export function revealedClip(story, progress, manifest, index = progress?.index, lang = 'en') {
  ...
  if (!node) return null;
  if (node.type === 'scene') {
    const sceneText = lang === 'en' ? node.en : lang === 'ru' ? node.ru : node.i18n?.line?.[lang];
